...
async function loadData() {
    try {
        ...
        if (!statsData.success || !collegesListData.success) {
            throw new Error(statsData.message || collegesListData.message || 'API returned error');
        }
...
async function loadCollegeDetails(colleges) {
    const collegePromises = colleges.map(async (college) => {
        try {
            const response = await fetch(`${API_BASE_URL}/api/college/${college.code}`);
            if (response.ok) {
...
function updateStatistics(data) {
    ...
    document.getElementById('total-colleges').textContent = summary.total_colleges.toLocaleString();
    document.getElementById('total-majors').textContent = summary.total_majors.toLocaleString();
    document.getElementById('total-applicants').textContent = summary.total_applicants.toLocaleString();
    document.getElementById('avg-applicants').textContent = summary.avg_applicants_per_major.toFixed(1);
}
...
function updateDataStatus(data) {
    ...
    if (data.last_updated) {
        ...
        lastUpdatedElement.textContent = `最后更新: ${updateTime.toLocaleString('zh-CN')}`;
    }
}
...
function createCollegeCard(college) {
    ...
    card.innerHTML = `
        ...
        <div class="college-code">代码: ${college.college_code || college.code}</div>
        <div class="college-stats">
            ...
            <div class="stat-item">
                ...
                <div class="stat-value">${(college.total_applicants || 0).toLocaleString()}</div>
            </div>
            <div class="stat-item">
                <span class="stat-label">平均申请</span>
                <div class="stat-value">${college.total_majors > 0 ? ((college.total_applicants || 0) / college.total_majors).toFixed(1) : '0'}</div>
            </div>
        </div>
    `;
...
function showCollegeDetail(college) {
    document.getElementById('modal-college-name').textContent = college.college_name || college.name;
    document.getElementById('modal-college-code').textContent = college.college_code || college.code;
    document.getElementById('modal-total-majors').textContent = college.total_majors || 0;
    document.getElementById('modal-total-applicants').textContent = (college.total_applicants || 0).toLocaleString();
    ...
    if (college.majors && college.majors.length > 0) {
        college.majors.forEach(major => {
            ...
            row.innerHTML = `
                ...
                <td class="${major.is_available === '是' ? 'available-yes' : 'available-no'}">
                    ${major.is_available}
                </td>
            `;
...
function sortColleges() {
    ...
    collegesData.sort((a, b) => {
        switch (sortBy) {
            ...
            case 'name-asc':
                return (a.college_name || a.name).localeCompare(b.college_name || b.name);
            case 'majors-desc':
