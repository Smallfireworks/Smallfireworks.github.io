...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const LS_INDEX = 'tsa_index_v1';
  const LS_META = 'tsa_meta_v1';
  const LS_COURSE = 'tsa_course_v1';
  ...
  function loadLocalCache() {
    try {
      ...
      const c = localStorage.getItem(LS_COURSE);
      if (i && m && c) {
        indexData = JSON.parse(i);
  ...
  function saveLocalCache() {
    try {
      if (indexData) localStorage.setItem(LS_INDEX, JSON.stringify(indexData));
      if (metadata) localStorage.setItem(LS_META, JSON.stringify(metadata));
      if (courseData) localStorage.setItem(LS_COURSE, JSON.stringify(courseData));
    } catch (e) {}
  }
  ...
  async function loadInitialData() {
    loadLocalCache();
    if (indexData && metadata && courseData) return;
    ...
    const skeletonCard = `
      <div class="skeleton-card" aria-hidden="true">
        <div class="teacher-header">
          ...
          <div class="skeleton-line dept shimmer"></div>
        </div>
  ...
  function indexMatches(teacher, keywords, deptTerm) {
    if (deptTerm && !(teacher.d && teacher.d.toLowerCase().includes(deptTerm))) return false;
  ...
  async function searchTeachers() {
    ...
    const keywords = tokenizeKeywords(searchInput.value);
    const deptTerm = (departmentInput.value || '').trim().toLowerCase();

    clearSearchBtn.classList.toggle('hidden', searchInput.value.length === 0);

    if (!indexData) return;

    if (!keywords.length && !deptTerm) {
    ...
    const matched = indexData.filter(t => indexMatches(t, keywords, deptTerm));
    ...
    function renderBatch() {
      ...
      const html = slice.map(idx => {
        ...
        return data ? renderTeacherCard(data, keywords) : '';
      }).join('');
  ...
  function highlightTerm(text, keywords) {
    if (!keywords || keywords.length === 0) return text;
    const joined = keywords.map(escapeRegExp).join('|');
    const regex = new RegExp(`(${joined})`, 'gi');
    return text.replace(regex, '<strong>$1</strong>');
  }

  function renderTeacherCard(t, keywords) {
    const hasGpa = Array.isArray(t.g) && t.g.length > 0;
    const hasComments = Array.isArray(t.m) && t.m.length > 0;

    const displayName = keywords?.length ? highlightTerm(t.n, keywords) : t.n;
    let html = `
    <div class="teacher-card" role="article" aria-labelledby="teacher-name-${t.id}">
      <div class="teacher-header">
        <span id="teacher-name-${t.id}" class="teacher-name">${displayName}</span>
        <span class="teacher-dept">${t.d || ''}</span>
      </div>
      <div class="teacher-stats">
        <span><b>评分:</b> ${(parseFloat(t.r) || 0).toFixed(2)} (${t.rc}人)</span>
        <span><b>热度:</b> ${t.h}</span>
      </div>
    `;

    if (hasGpa) {
      html += `
      <button type="button" class="collapsible" aria-expanded="false" aria-controls="gpa-${t.id}" data-cid="${t.id}">课程GPA信息 (${t.g.length})</button>
      <div class="collapsible-content" id="gpa-${t.id}"><div class="content-inner">
        ${renderGpaTable(t)}
      </div></div>`;
    }
    if (hasComments) {
      ...
      html += `
      <button type="button" class="collapsible" aria-expanded="false" aria-controls="comments-${t.id}" data-cid="${t.id}">学生评价 (${t.m.length})</button>
      <div class="collapsible-content" id="comments-${t.id}" data-teacher-id="${t.id}">
        <div class="content-inner">${renderCommentsContainer(t, sortBy)}</div>
      </div>`;
    }

    if (!hasGpa && !hasComments) {
      html += `<p style="text-align:center;color:#888;padding:15px 0;">暂无课程GPA或学生评价数据</p>`;
    }
  ...
  function renderGpaTable(t) {
    return `
      <table class="gpa-table" aria-label="课程GPA">
        <thead><tr><th>课程名</th><th>平均绩点±标准差</th><th>选课人数</th></tr></thead>
        <tbody>
          ${t.g.map(c => `
            <tr>
              <td><a href="#" class="course-link" data-course-name="${c[0]}" data-teacher-name="${t.n}">${c[0]}</a></td>
              <td>${(parseFloat(c[1])||0).toFixed(2)} ± ${(parseFloat(c[3])||0).toFixed(2)}</td>
              <td>${c[2] === 0 ? '500+' : c[2]}</td>
            </tr>
          `).join('')}
        </tbody>
  ...
  function renderCommentsContainer(teacher, sortBy = 'popularity') {
    const comments = teacher.m || [];
    const controls = `
      <div class="comment-controls">
        <span class="comment-count-display">共 ${comments.length} 条</span>
        <div class="comment-sort-buttons">
          <button class="sort-btn ${sortBy === 'popularity' ? 'active' : ''}" data-sort="popularity">按人气</button>
          <button class="sort-btn ${sortBy === 'time' ? 'active' : ''}" data-sort="time">按时间</button>
        </div>
      </div>`;
  ...
  function renderSortedComments(comments, sortBy) {
    const sorted = [...comments].sort((a, b) => {
      if (sortBy === 'time') return (b.t || '').localeCompare(a.t || '');
      return (b.p || 0) - (a.p || 0);
    });
    return sorted.map(c => `
      <div class="comment">
        <div class="comment-body" style="white-space: pre-wrap;">${(c.c || '').replace(/\n/g, '\n')}</div>
        <div class="comment-meta">
          <span>${c.t || ''}</span>
          <span class="comment-likes">
            <span class="like">👍 ${c.u || 0}</span> | <span class="dislike">👎 ${c.d || 0}</span>
          </span>
        </div>
      </div>`).join('');
  }
  ...
  function showCourseModal(courseName, currentTeacherName) {
    ...
    const renderCount = (n) => n === 0 ? '500+' : n;

    const rows = [...courseInfo].sort((a,b) => (parseFloat(b.g)||0) - (parseFloat(a.g)||0)).map(t => `
      <tr class="${t.t === currentTeacherName ? 'highlight-row' : ''}">
        <td>${t.t}</td>
        <td>${(parseFloat(t.g)||0).toFixed(2)} ± ${(parseFloat(t.s)||0).toFixed(2)}</td>
        <td>${renderCount(t.n)}</td>
      </tr>`).join('');

    modalContainer.innerHTML = `
      <div class="modal-overlay visible" role="dialog" aria-modal="true" aria-labelledby="${modalId}">
        <div class="modal-content">
          <div class="modal-header">
            <h2 class="modal-title" id="${modalId}">${courseName}</h2>
  ...
  function updateDepartmentDropdown() {
    ...
    departmentDropdown.innerHTML = filtered.map((dept, i) => `
      <div class="dept-dropdown-item ${i===0?'active':''}" role="option" data-value="${dept}">${dept}</div>
    `).join('');
    deptActiveIndex = 0;
  ...
  document.addEventListener('click', (e) => {
    if (!departmentInput.contains(e.target) && !departmentDropdown.contains(e.target)) {
  ...
  departmentInput.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowDown') { e.preventDefault(); moveDeptActive(1); }
      ...
      const active = departmentDropdown.querySelector('.dept-dropdown-item.active');
      if (active) {
        departmentInput.value = active.dataset.value;
        departmentDropdown.classList.add('hidden');
        searchTeachers();
      }
  ...
  teacherList.addEventListener('click', e => {
    ...
    if (target.classList.contains('collapsible')) {
      ...
      const idx = indexData.find(t => t.id === teacherId);
      if (!idx) return;
      const teacher = loadedTeachers.get(idx.c)?.find(x => x.id === teacherId);
      if (!teacher) return;
  ...
  modalContainer.addEventListener('click', e => {
    if (e.target.matches('.modal-overlay, .modal-close, .modal-close *')) closeModal();
  });
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape' && !modalContainer.classList.contains('modal-hidden')) closeModal();
  });
